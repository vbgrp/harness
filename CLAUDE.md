...
4. Embed — 블로그 메인 페이지(index.html)에 웹앱 카드를 추가한다. 카드에는 제목, 설명, 미리보기 이미지 또는 iframe을 넣는다. 새 카드는 `.app-grid`의 맨 앞(좌상단)에 삽입해 최신 앱이 가장 먼저 보이도록 한다(기존 카드들은 뒤로 밀림). 깃 커밋한다.
